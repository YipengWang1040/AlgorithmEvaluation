...
def parse_results(result_dir, frame_id = "world"):
    with open(result_dir + "/results.json") as f:
        definations = json.load(f)

    print("Found {} algorithms and {} dataset with {} sequences in total".format(len(definations["algorithms"]),
        len(definations["datasets"].keys()), len([seq for dataset in definations["datasets"].values() for seq in dataset["sequences"]])))
    
    ret={}
    # all algorithms
    for algorithm in definations["algorithms"]:
        ret[algorithm] = {}
        algorithm_dir = result_dir+'/'+algorithm+"_results"
        for dataset_name in definations["datasets"].keys():
            ret[algorithm][dataset_name]={}
            dataset = definations["datasets"][dataset_name]
